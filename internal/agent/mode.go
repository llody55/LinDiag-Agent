...
// outputFormatRule 是所有模式共用的输出格式约束
const outputFormatRule = "\n\n" +
	...
	"- 用 \"**粗体**\" 标注关键数据\n" +
	"- 用三反引号代码块包裹命令和输出引用\n" +
	"- 数据必须标注来源，例如：\"根据 `df -h` 输出：根分区已用 37G（53%）\"\n" +
	"- 禁止输出无格式的纯文本大段文字\n" +
	"\n" +
	"示例格式：\n" +
	"### 诊断结论\n" +
	...
	"\n" +
	"### 处理建议\n" +
	"1. 步骤一（命令：`du -sh /var/lib/docker`）\n" +
	"2. 步骤二"

// === 内置诊断模式实现 ===

// baseSnapshotCmds 所有模式共享的基础快照命令集。
// 主题分块：基础资源 → 进程/负载 → 内核日志 → 磁盘详情 → 僵尸进程。
// 所有命令强制截断输出 + 2>/dev/null 容错，避免：
//  1. 某条命令在特定环境缺失时报错淹没上下文
//  2. 单条命令输出过大使 system prompt 超长导致 LLM 丢上下文
//
// 按环境动态追加 Docker / K8s 上下文。
func baseSnapshotCmds(env *EnvInfo) []string {
	cmds := []string{
		// 基础资源：CPU负载/内存/磁盘
		"uptime", "free -h", "df -h",
		// 进程 TOP15：按内存排序，覆盖 CPU/MEM 异常的元凶进程
		"ps -eo pid,ppid,%cpu,%mem,rss,cmd --sort=-%mem | head -n 15",
		// top 单次快照：包含系统级 load average、CPU 占用分布
		"top -b -n 1 | head -n 20",
		// 内核环形缓冲区最后 30 行：OOM、panic、硬件错误、驱动告警
		"dmesg 2>/dev/null | tail -n 30",
		// 文件系统 inode 使用率（与 df -h 块使用率互补，定位"磁盘没满但写不进"问题）
		"df -i 2>/dev/null | head -n 15",
		// 僵尸进程计数（ zombie > 0 通常意味着父进程未回收子进程 ）
		"ps -eo stat,pid,cmd 2>/dev/null | awk '$1 ~ /Z/ {print}' | head -n 10",
		// 系统服务失败计数（systemd 环境）定位启动失败的服务
		"systemctl --failed --no-legend 2>/dev/null | head -n 15",
	}
	if env == nil {
		return cmds
	}
	// 容器上下文：仅在 Docker 运行时采集（避免 docker 未装时一堆报错）
	...
	// 异常服务状态：对每个异常服务采集 systemctl status，给 AI 提供失败原因证据。
	// （env.AbnormalServices 也通过 systemPrompt 注入为"优先诊断方向"，此处只负责取证据）
	for _, svc := range env.AbnormalServices {
		// systemctl status 输出含失败原因行（Active: failed (Result: exit-code)），
		// 限制行数避免单个服务输出过长
		cmds = append(cmds, fmt.Sprintf("systemctl status %s --no-pager 2>/dev/null | head -n 15", svc))
	}
	return cmds
}

// faultDiagnosisMode 故障诊断模式（专业）
...
func (m *faultDiagnosisMode) SnapshotCmds(env *EnvInfo) []string {
	// 故障诊断模式：采集足以支撑根因定位的系统全貌。
	// 按"主题分块 + 强制截断"组织：避免单条命令输出过大淹没上下文，
	// 也避免某条命令在特定环境缺失时报错拖累整体采集。
	cmds := baseSnapshotCmds(env)
	// 故障诊断模式追加更深的网络/内核状态
	cmds = append(cmds,
		// 内核模块与参数：定位模块丢失、参数漂移类问题
		...
		// TCP 连接分布：定位 TIME_WAIT 堆积、CLOSE_WAIT 异常、SYN_RECV 攻击
		"ss -tan 2>/dev/null | awk 'NR>1{print $1}' | sort | uniq -c | sort -rn",
		// 路由表：定位多网卡/default 路由缺失、策略路由问题
		"ip route show 2>/dev/null | head -n 20",
		// 连接跟踪表规模：定位 conntrack 表打满导致丢包
...
func (m *smartMode) MaxRounds() int { return 0 } // 无限循环
...
func (m *smartMode) SnapshotCmds(env *EnvInfo) []string {
	// 智能模式：用基础快照集即可；不需要故障诊断模式的深度网络/内核命令
